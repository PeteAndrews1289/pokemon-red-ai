...
A trainer-side V8 verification pass that concatenates two or more locally competent,
self-generated skill traces and replays them continuously from exact power-on. Every protected
endpoint must match before any transition excerpt may train the Student. A successful replay is
training-data provenance, not Student competence: only a later frozen composition exam tests
whether the Student can choose those actions itself. V8 keeps only the current deepest verified
composition active in replay and preserves earlier audit records as inactive history.
Composition endpoint checks use an exact processed-visual-plus-gameplay-RAM signature that remains
stable across emulator save/load; PyBoy's broader derived game-area hash did not. Distillation can
retain that stricter hash because it compares candidates replayed from the same snapshot.
...
A path-free, read-only snapshot used to keep a comparison baseline from moving after a new run
starts. Fresh V8 can match a V7 self-taught checkpoint to its latest or previous model hash and
seal only the run ID, Explorer actions, best milestone, model/checkpoint hashes, state, and
timestamps. Resume reuses that manifest record instead of rereading a newer V7 state.
...
### Distillation / trajectory distillation

Turning a larger body of experience into a smaller training artifact. In Version 8, the input is
an action trace generated by the run itself. Loops or chunks may be removed only when replay from
the same source still reaches the protected outcome. Distillation can remove accidental wandering;
it does not prove that the remaining route is optimal or understood.
...
The Version-8 recurrent PPO policy that searches for new consequences in four simultaneous games.
It can create candidate experience but cannot declare a skill competent. Its parameters and
optimizer are separate from the Student that studies verified discoveries.
...
A predeclared attempt during which model parameters and optimizer state do not change. V8 records
exactly one deterministic grade per Student checkpoint, then waits for a later Student version
before adding another grade to its rolling window. Local exams start from a skill's training source;
stronger power-on composition exams test connected behavior without snapshot restoration. All
planned failures belong in the denominator. In V8 composition, a trainer-side RAM referee switches
an ordered playlist of the run's self-generated visual goals at declared milestones. One frozen
Student still chooses every button, but the result is goal-conditioned hierarchical control—not
unaided pixel-only autonomy—and its goal-switching protocol must be named.
...
A record of which self-generated skill must be competent before another is eligible for practice
or composition. In V8 the edges come from the Explorer's verified lineage, not a human walkthrough.
The scheduler is still supplied training structure and is not evidence that the policy inferred a
quest plan.
...
An immutable, hash-bound V8 training derivative created when a self-generated skill is admitted.
It owns a disjoint contiguous range of at most 512 loss-bearing examples and may prepend up to one
configured burn-in window solely to rebuild recurrent state. A persistent per-skill cursor loads
one shard per routine Student round and covers every shard across resume. The original full skill
NPZ remains provenance-only during routine replay and is still checked during resume/full audit.
...
The Version-8 recurrent policy trained only on replay-verified, self-generated, distilled skill
sequences. It has a separate optimizer from the PPO Explorer. Falling Student loss is practice
evidence; frozen attempts determine whether it can reproduce a skill.
